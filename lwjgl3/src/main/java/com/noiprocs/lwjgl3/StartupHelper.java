...
/**
 * A helper object for game startup, featuring three utilities related to LWJGL3 on various
 * operating systems.
 *
 ...
 *   <li>macOS: Spawns a child JVM process with {@code -XstartOnFirstThread} in the JVM args (if it
 *       was not already). This is required for LWJGL3 to work on macOS.
 *   <li>Linux (NVIDIA GPUs only): Spawns a child JVM process with the {@code
 *       __GL_THREADED_OPTIMIZATIONS} {@link System#getenv(String) Environment Variable} set to
 *       {@code 0} (if it was not already). This is required for LWJGL3 to work on Linux with NVIDIA
 *       GPUs.
 * </ul>
 *
 * <a
...
public class StartupHelper {
   ...
   *
   * <p>All {@link System#getenv() Environment Variables} are copied to the child JVM process (if it
   * is spawned), as specified by {@link ProcessBuilder#environment()}; the same applies for {@link
   ...
   * </code></pre>
   *
   * @param inheritIO whether I/O should be inherited in the child JVM process. Please note that
   *     enabling this will block the thread until the child JVM process stops executing.
   * @return whether a child JVM process was spawned or not.
   */
  public static boolean startNewJvmIfRequired(boolean inheritIO) {
    ...
    if (osName.contains("windows")) {
      ...
      // By default, LWJGL3 extracts to the directory specified by "java.io.tmpdir": usually, the
      // user's home.
      ...
      // avoid this.
      // We also temporarily change the "user.name" property to one without any chars that would be
      // invalid.
  ...
  public static boolean startNewJvm0(boolean isMac, boolean inheritIO) {
    ...
    if (!isMac) {
      ...
      boolean isMainThread =
          JNI.invokePPZ(currentThread, ObjCRuntime.sel_getUid("isMainThread"), objcMsgSend);
    ...
    // This state shouldn't usually be reachable, but this stops us from endlessly spawning new
    // child JVM processes.
